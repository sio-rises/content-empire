  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <section className="mb-12">
        <h1 className="text-3xl font-bold tracking-tight text-gray-900">NISA投資入門</h1>
        ...
        <div className="grid gap-6">
          {articles.map((article) => (
            <a key={article.slug} href={`/articles/${article.slug}`} className="group rounded-xl border border-gray-100 bg-white p-6 shadow-sm transition-shadow hover:shadow-md">
              <h2 className="text-xl font-bold text-gray-900 group-hover:text-[#10b981]">{article.title}</h2>
              <p className="mt-2 text-sm text-gray-500">{article.metaDescription}</p>
              <div className="mt-3 flex flex-wrap gap-2">
                {article.keywords.map((k) => <span key={k} className="rounded-full bg-green-50 px-2.5 py-0.5 text-xs text-[#10b981]">{k}</span>)}
              </div>
